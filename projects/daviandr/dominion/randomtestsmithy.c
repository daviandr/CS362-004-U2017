...
*/
	...
	if(sizeDiff == 2)
	{
		printf("TEST PASSED: 2 more card drawn\n");
	}
	else
	{
		printf("TEST FAILED\n");
		printf("	Hand Count before card: %d\n", oldHand);
		printf("	Hand Count after card: %d\n", g->handCount[cp]);
	...
	if(sizeDiff == 3)
	{
		printf("TEST PASSED: Correct deck size after card drawn\n");
	}
	else
	{
		printf("TEST FAILED:\n");
		printf("	Deck count before card: %d\n", oldDeck);
		printf("	Deck count after card: %d\n", g->deckCount[cp]);
	}
	//assert(sizeDiff == 3);
